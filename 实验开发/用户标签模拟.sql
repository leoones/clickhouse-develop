...
where tag_name =  'A1009'
order by rand() limit 23876032
    ;

alter table tag_dt.ch_tag_user_date drop partition 'A1002';
...
---1.7 性别 A1001
    ...
    alter table tag_dt.ch_tag_user_str drop partition 'A1001';
    insert into tag_dt.ch_tag_user_str(label_id,
                                       label_value,
                                       users)
       select  tag_name, tag_value,
...
-----------------------------------------------------------------------支付行为---------------------------------------------------------------------
  -- 2.1 首单时间 A2001
    ...
    select ctum.userid,
           'A2001',
           addDays(toDate(ctum.tag_value) , modulo(rand(), 365))
      from tag_dt.ch_user_tag_info ctum
    where ctum.tag_name = 'A1009'
    order by rand() limit  13738974;
  ...
  ---2.8 首单支付时间 A2008
   ...
   insert into tag_dt.ch_user_tag_info(userid,
    ...
    select ctum.userid,
      ...
      from tag_dt.ch_user_tag_info ctum
    where ctum.tag_name = 'A2001'
    order by rand64() limit 12798278;
  ...
  ---2.8 首单支付金额 A2009
   ...
   insert into tag_dt.ch_user_tag_info(userid,
    ...
    select tmp1.userid,
      ...
      from
            (select  *
              from tag_dt.ch_user_tag_info ctum
           where ctum.tag_name = 'A2008') tmp1
        inner join (select  *
              from tag_dt.ch_user_tag_info ctum
           where ctum.tag_name = 'A2002') tmp2
   ...
   --2.9 支付总笔数
    ...
    select tmp1.userid,
           'A2010',
           toUInt64OrZero(tmp2.tag_value) - modulo(rand64(),2)
      from
            (select  *
              from tag_dt.ch_user_tag_info ctum
           where ctum.tag_name = 'A2008') tmp1
        inner join (select  *
              from tag_dt.ch_user_tag_info ctum
           where ctum.tag_name = 'A2003') tmp2
        on tmp1.userid = tmp2.userid
        ;
  ...
  --2.10 支付总金额
    alter table tag_dt.ch_user_tag_info drop partition 'A2011';
    ...
    insert into tag_dt.ch_user_tag_info(userid,
        ...
        select tmp1.userid,
               'A2011',
               toUInt64(tmp2.tag_value) - modulo(rand64(), 50)
          from
                (select  *
                  from tag_dt.ch_user_tag_info ctum
               where ctum.tag_name = 'A2008') tmp1
            inner join (select  *
                  from tag_dt.ch_user_tag_info ctum
               where ctum.tag_name = 'A2005') tmp2
            on tmp1.userid = tmp2.userid
            ;
   ...
   insert into tag_dt.ch_user_tag_info(userid,
    ...
    select tmp1.userid,
           'A2012',
           toUInt64(tmp2.tag_value) - modulo(rand64(), 5)
      from
            (select  *
              from tag_dt.ch_user_tag_info ctum
           where ctum.tag_name = 'A2008') tmp1
        inner join (select  *
              from tag_dt.ch_user_tag_info ctum
           where ctum.tag_name = 'A2004') tmp2
        on tmp1.userid = tmp2.userid
        ;
 ...
 --2.12 最近支付时间
    ...
    select userid,
           'A2013',
           addDays(toDate(ctum.tag_value), modulo(rand64(), 365))
      from tag_dt.ch_user_tag_info ctum
       where ctum.tag_name = 'A2008'
        ;

    alter table tag_dt.ch_tag_user_date drop partition 'A2014';
    insert into tag_dt.ch_tag_user_date(label_id,
                                       label_value,
                                       users)
       select  tag_name, tag_value,
...
--------------------------------------------------3. 用户行为标签 --------------------------------------------------
 ...
 --3.2 最近签到时间 A3002
     ...
     insert into tag_dt.ch_user_tag_info(userid,
        ...
        select  userid,
               'A3002',
               addMonths(toDate(tag_value),modulo(rand64(), 365))
          from tag_dt.ch_user_tag_info
         where tag_name = 'A1009'
       order by  rand() limit 5866807 ;

    alter table tag_dt.ch_tag_user_date drop partition 'A3002';
    insert into tag_dt.ch_tag_user_date(label_id,
                                       label_value,
                                       users)
       select  tag_name, tag_value,
